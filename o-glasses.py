...
def main():
    ...
    if not args.input_model:
        ...
        # '/home/nozima/projects/bin-gen/new-bins/data/train'
        # '/home/hohyeon/binshot/new_bin/train'
        path = '/home/nozima/projects/bin-gen/new-bins/data/train'

        # ファイル一覧の取得

        files_file = [f for f in os.listdir(path)]
        # ファイルタイプのナンバリング
        file_types = {}
        file_types_ = []
        num_of_file_types = {}
        ...
        for f in files_file:
            # ディレクトリ名でファイルタイプ分類
            file_type = os.path.basename(f)
            # f.replace(path, "").replace(
            #     os.path.basename(f), "").split("-", 1)[0]
            # print(file_type)
            if 'gcc' in file_type:
                file_type = 'gcc'
            elif 'clang' in file_type:
                file_type = 'clang'
            else:
                file_type = 'nim'
            #file_type = file_type.split('-')[-2]
        ...
        for f in files_file:
            ...
            for c in range(0, fsize-block_size, block_size):
                ...
                train = np.array([np.float32(x/255.0)
                                 for x in block])
    ...
    if args.output_model and master_dataset:
        ...
        if not args.input_model:
            ...
            # Entries other than 'epoch' are reported by the Classifier link, called by
            # either the updater or the evaluator.
            trainer.extend(extensions.Print(Report()
        ...
        img = Image.new('RGB', (128, int((fsize+127)/128)))
        ...
        for c in range(0, fsize-block_size, l):
            offset = c*1.0/fsize
            block = bdata[c:c+block_size]
            block_ = [hex(x) for x in block]
            e = entropy(block_)
            for j in range(0, l):
                img.putpixel(((c+j) % 128, int((c+j)/128)), (e, e, e))
                if args.disasm_x86:
            ...
            train = np.array([np.float32(int(bitmap_view(hex(x)), 16)/255.0)
                             for x in block])
            #train = np.append(train,np.float32(offset))
        ...
        for i in range(len(checked_dataset)):
            with chainer.using_config('train', False):
                result = model.predictor(np.array([checked_dataset[i]]).astype(
                    np.float32)).data.argmax(axis=1)[0]
            results[result] += 1
            for j in range(0, l):
                img.putpixel(((i*l+j) % 128, int((i*l+j)/128)), col[result])
        print('Results:', results, file_types_[get_result(results)])
        img.save("v.png")
        # img.show()
    else:
        # k-分割交差検証
        ...
        for i in range(k):
            ...
            frequency = args.epoch if args.frequency == - \
                1 else max(1, args.frequency)
